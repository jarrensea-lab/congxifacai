...
当前 feature 分支版本 `v9.0.0-dev`，核心方向是“账户可执行机会 + 主动发现 + 阶段自愈”：主报告先回答持仓和次日动作，再列最多三只当前现金可以买得起且通过硬门的标的；每次运行记录发现、过滤、补数、评分、生命周期、报告和交付状态。预测账本继续沉淀 T+1/T+3/T+5 样本，真实执行复盘约束下一次策略。迭代方向见 [ROADMAP.md](ROADMAP.md)。
...
20:30 ── v9 次日投资策略主管线（周一至周五；周日服务周一）
20:45 ── v9 断点恢复 + 易淘金账户→自选同步（后者可选，默认关闭）
21:00 ── Sentinel 绩效回看与归档
21:15 ── v9 报告交付验真
```

所有 APScheduler 作业由 `backend/app/services/scheduler_service.py` 统一登记和启动；`backend/app/main.py` 只注入处理函数。启动时会清理遗留的 `daily_report` 作业，并只在当天已经存在未完成运行时恢复，不凭空重复创建任务。

---

## v9 可执行机会与自愈报告

v9 每晚先读取真实现金和总资产，计算现金安全垫、单票预算和最小交易单位，然后执行：

```text
账户快照 → 全市场发现 → 可负担过滤 → 数据补齐 → 综合评分
→ 生命周期写入 → 报告渲染 → 语义校验 → 交付 → 文件摘要验真
```

- **主动发现**：发现与评分在同一次运行内完成，不再只循环旧标的池。
- **先判断买不买得起**：一手金额超过预算的股票不进入正文前三名，也不会生成买入数量。
- **解释变化**：新标的显示评分、前三条理由、主要风险、价格、股数、止损、目标和最大计划亏损；新增、降级、剔除逐只列明原因。
- **旧标的每天复评**：已有标的不会因跌出当日排名而失联；首次跌破 65 分留池线降级，连续两次跌破或中长期逻辑红线触发则剔除，已剔除标的不自动复活。
- **没有建议也要交代**：报告显示扫描数、买得起数量、完成评分数量、没有买入建议的具体原因和最接近触发的观察标的。
- **阶段自愈**：SQLite 记录十个阶段、尝试次数、产物摘要和错误码；外部发现为空最多重试三次，恢复任务从首个未完成阶段继续，已完成的生命周期写入不会重复。
- **语义验收**：文件存在不等于成功。报告必须包含账户、可操作结论、完整变化和管线状态；可操作卡片最多三张，交付文件摘要必须匹配。

`v9.0.0-dev` 仍然只提供研究、策略、风控和人工执行建议。它不会自动下单、撤单、转账，也不会绕过券商端确认。
...
│       │   ├── opportunity_pipeline.py # v9 主动发现、评分、恢复和交付编排
│       │   ├── pipeline_run_store.py # v9 SQLite 阶段账本
│       │   ├── composite_score.py  # 六维 100 分可解释评分
│       │   ├── target_lifecycle_events.py # 新增/降级/剔除事件
│       │   ├── report_validation.py # 报告语义验收
...
- **数据快照归一**：每只标的聚合行情、K 线、资金流、龙虎榜、大单、北向、新闻/公告、财务、Sentinel 证据和 Serenity 深挖。
...
- **本地离线分钟档案** — 通过 `data/external_market_data_sources.json`
  ...
  该来源强制为 `shadow`，可供回测、预测补样、策略实验和日报历史 K 线兜底，
  但始终标记为 `historical_only`，不能替代实时行情或当日收盘价。
...
文件有效时，历史回测优先读取离线数据；日报则优先使用在线增量，仅在在线 K 线
不可用时把离线数据作为历史基线，并用 Tushare 复权因子计算前复权（qfq）。读取器同时
