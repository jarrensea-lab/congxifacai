...
⚠️ 铁律：禁止推荐候选池外的股票！只从市场数据中提供的候选池选择推荐标的。如果候选池中没有值得关注的标的，在recommendations中填写"数据不足，建议观望"，不要去池外另找。
⚠️ 铁律：必须严格尊重账户预算！对于¥5,000以下小账户，只推荐1手价格(股价×100)不超总资产的股票。如果池内股票全部超预算，写"数据不足，建议观望"，严禁推荐买不起的股票。
...
⚠️ 铁律：裁判必须过滤掉所有池外推荐！只采纳四位专家中针对候选池内股票的推荐。如果专家推荐了池外股票，在reasoning中注明"专家推荐的XXX不在候选池内，已过滤"。
