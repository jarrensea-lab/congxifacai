...
- [x] 主报告第一屏以持仓处置和次日动作优先，动作模板已迁移到 `backend/app/report_engine/templates/next_day.py`；`scripts/daily_report.py` 只负责取数、编排和落盘。
...
- [x] 中线/长线建议不再整段缺席，状态明确为 `unknown`、`stale` 或 `healthy`；长期质量只能约束和解释交易，不能单独授权买入。
...
- [x] DeepSeek/Qwen 按运行时配置路由；Qwen 缺失会留下显式 fallback 元数据，没有可用 provider 或结果降级时不写入生产候选池。
...
- [ ] 为 `unknown/stale/healthy` 长期状态建立季度复核队列和过期提醒，继续保持 research-only。
...
- 当前后端完整基线：`1099 passed`（2026-07-26）。
...
| 模型缺失或降级 | 多角色观点不完整 | 路由元数据、降级标记、生产候选写入 fail-closed |
| 离线大数据损坏或混入未来数据 | 回测失真 | shadow-only、归档边界、时间戳/重叠/复权完整性校验 |
| 中长期证据过期 | 旧 thesis 误导决策 | `unknown/stale/healthy` 状态、红线和复核任务 |
