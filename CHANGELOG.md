...
- `PYTHONPATH=backend:. .venv/bin/pytest -q backend/tests`：`1262 passed`。
- `.venv/bin/ruff check backend scripts`：通过；`git diff --check`：通过。
- 隔离临时路径真实数据验收：扫描 `5197` 只，可负担 `1322` 只，限制补齐和评分 `2` 只；十个阶段全部完成，报告语义校验通过，生成报告存在且操作卡数量为 `0`（当轮没有标的通过全部买入硬门）。
