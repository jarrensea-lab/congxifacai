...
## Unreleased (2026-07-31) — v9 可执行机会与自愈报告

### 用户结果

- ✅ **账户可执行优先**：先按可用现金、现金安全垫、单票预算和最小交易单位过滤，再对可负担标的补数和评分；买不起的股票不进入正文推荐。
- ✅ **每天主动发现**：全市场发现、新标的数据补齐和综合评分在同一次运行完成，不再依赖“先评分旧池、下一天才看见新股”的断链流程。
- ✅ **简洁策略首屏**：最多展示三只可操作标的，包含级别、综合评分、三条主要理由、主要风险、入场价、股数、金额、止损、目标和最大计划亏损。
- ✅ **变化可审计**：新增、降级和剔除逐只显示原因；没有买入建议时显示扫描、可负担、已评分数量和最接近触发条件。

### 管线与恢复

- ✅ **十阶段 SQLite 账本**：账户、发现、过滤、补数、评分、生命周期、渲染、校验、交付和验真分别记录状态、尝试次数、数据时间、产物路径、摘要和错误码。
- ✅ **有界自愈**：市场发现空结果最多重试三次；支持备用源降级；20:45 从首个失败或未完成阶段恢复，21:15 校验报告文件与摘要；启动恢复只处理已存在的当天运行。
- ✅ **幂等生命周期**：恢复报告渲染或交付时不会重复应用标的池变更；确定性评分证据通过全部硬门后，动态发现标的可晋级为 `executable`。
- ✅ **标的池每日复评**：已有标的即使跌出当日市场排名也会强制进入复评；首次跌破 65 分留池线降级，连续两次跌破或中长期逻辑红线触发则剔除，已剔除标的不会被普通发现自动复活。
- ✅ **语义成功标准**：空市场、隐藏生命周期变化、缺少“不买”原因、超过三张操作卡或交付摘要不匹配均不能标记为成功。
- ✅ **数据健康修复**：空的 Sentinel/Serenity 对象改为 `missing/empty_payload`，不再伪装成健康证据。
- ✅ **本地历史正式接入**：已注册的分钟历史与 Tushare 前复权因子进入日常 K 线门面；在线增量优先，在线 K 线失败时才使用本地历史，并显式标记 `historical_only`，不冒充当日行情。
- ✅ **市场证据实际消费**：行业/概念资金、市场新闻、龙虎榜和大单追踪进入主分析与单股快照；龙虎榜和大单可作为资金评分的交叉确认，报告审计显示实际消费数量。
- ✅ **供应源自愈**：腾讯指数支持有界重试和逐代码恢复；报告日官方收盘价按有效收盘口径验真；AKShare JavaScript 解析器改为安全串行调用，避免并发初始化导致进程退出。
- ✅ **假缺口清理**：当日全市场清单不存在的旧研究代码自动剔除，不再写成“关键数据未补齐”；Sentinel 主源空结果会切换 AKShare 新闻，零事件不能伪装成功。
- ✅ **预算票静默过滤**：买不起最小交易单位的股票不再逐只出现在“今日变化”或预算附录，也不留在活跃观察池；主报告只保留过滤数量，内部历史降为静默研究参照。

### 评分与安全边界

- ✅ **六维 100 分评分**：量价趋势 30、资金 20、行业催化 15、基本面估值 15、相对强度 10、风险收益 10；高分仍不能绕过预算、数据、交易剧本、市场、追高和风险硬门。
- ✅ **无自动交易**：本版本没有增加自动下单、撤单、转账或券商交易授权；所有真实操作继续由用户人工决定。

### 验证

- `PYTHONPATH=.:backend .venv/bin/python -m pytest -q backend/tests`：`1283 passed`。
- `.venv/bin/ruff check backend scripts`：通过；`git diff --check`：通过。
- 隔离临时路径真实数据验收：扫描 `5197` 只，可负担 `1281` 只，完成评分 `33` 只；消费行业/概念 `50` 项、市场新闻 `35` 条、龙虎榜 `20` 条、大单 `30` 条；十个阶段全部完成，报告语义校验通过，当轮没有标的通过全部买入硬门。
- 验收使用临时持仓副本、候选池、报告目录和管线数据库，并关闭飞书推送；没有改写真实持仓、真实候选池或真实报告归档。
- 核验后将同一摘要的正式报告覆盖归档并通过飞书 Webhook 推送成功；真实持仓和真实候选池未被隔离验收改写。
...
### 策略与复盘
- ✅ **AI 生产写入 fail-closed**：质量校验异常、空响应、角色降级或裁判降级时，推荐只保留在快照/报告审计中，不写入生产候选池。
...
### 策略主线
- ✅ **主报告终极降噪**：次日投资策略主报告第一屏改为“明日唯一实盘狙击标的”，优先回答干不干、干谁、买入逻辑、触发价、一手金额、止损和第一目标；买不起的标的不再展示名称，只保留预算阻断数量。
...
- ✅ **次日条件策略补强**：当标的池没有可执行买入时，主报告会把小账户低价候选首选升格为“盘前重点复核标的”，包含买入逻辑、建议试仓金额、触发价、止损位和第一目标。
...
- ✅ 新增 `backend/app/services/small_account_discovery.py`：维护低价高流动性种子池，按账户现金计算账户可买上限价。
- ✅ 新增最小交易单位规则：主板/创业板默认 100 股，科创板 `688/689` 默认 200 股。
- ✅ 日报生成时会从 Target Pool 拉取标的评分，并把评分结果回写标的池状态。

### 报告与运行
- ✅ `scripts/daily_report.py` 默认输出 v7.5 盈利导向结构：明日唯一实盘狙击标的、明日盘中雷达触发池、持仓与市场风控、后台风控与策略审计、数据覆盖与评分审计、复盘、研究归档；旧长报告仅通过 `CONGXI_REPORT_LEGACY_SECTIONS=1` 保留。
- ✅ `scripts/daily_report.py` 将池外小账户候选作为策略线索处理：买得起且有量能线索的候选进入实盘狙击或雷达触发池；买不起的统一隐藏为“预算阻断 N 只”，不再和可交易标的混排。
...
### 项目审查与清理
- ✅ 删除 knowX/教程残留：`config.json`、`data/mypkg/*`、`data/test_utils.py`、`scripts/knowx-news.sh`、`scripts/polling-agent.sh` 等。
- ✅ 删除旧误导入口：`backend/debate_pool.py`、`backend/test_reports.py`、`scripts/guardian.plist`、`scripts/start_all.sh`。
- ✅ `AGENTS.md` 与 `SYSTEM.md` 从 knowX 身份改为恭喜发财项目规则。
- ✅ `scripts/guardian.sh` 修正为 v7/8000 监控脚本。
...
### Feature 方向
- 🚧 **候选标的池生命周期**：从“每天重新扫描”改为“池外发现、池内复核、符合升级、不符合出池”。
- 🚧 **持仓管理池生命周期**：持仓必须携带买入理由、止损、止盈、移动止损、失效条件和复核时间。
- 🚧 **定时/手动触发闭环**：先用盘前、午盘、午后、收盘任务和手动运行把架构、数据、策略、监控、预警跑通；分钟/秒级后置。
- 🚧 **小账户盈利优先**：小账户不再默认保守，而是高收益验证账户；大账户才进入保守复利和分层资金管理。
- 🚧 **Sentinel/Serenity 入池**：研究包不再只展示在报告里，必须转化为候选池事件和后续复核任务。

### 已落地
- ✅ 新增 `backend/app/services/quant_lifecycle.py`：生产候选池 `candidate_pool.json` 与持仓监控池 `position_watch.json`。
- ✅ 盘前推荐写入候选池；午盘/午后空仓也扫描候选池，触发后通过飞书提醒。
- ✅ 午后持仓风控读取 `position_watch` 的止损价/目标价，触发止损或止盈提醒。
- ✅ 新增生命周期单元测试，覆盖数据不足入池、禁止追、可试仓、持仓止盈止损和告警等级归一。

### 怀疑性 Review 结论
- 当前系统没有真正的长期标的池，`stock_pool` 只是当次辩论临时结果。
- 空仓时盘中任务直接推“空仓观望”，没有扫候选池。
- 持仓风控没有吃到止损价、目标价和交易计划，导致止盈止损检查无法可靠触发。
- README 与 FastAPI health 版本存在不一致，feature 分支统一到 `v7.4.0-dev`。

### 文档
- 新增设计文档：`docs/superpowers/specs/2026-07-01-quant-lifecycle-v7-4-design.md`。
...
- ✅ **风险边界更新**：账户最大回撤 `-10%`，单票上限 `50%`，现金底线临时降至 `10%`，允许低价高波动标的，但默认排除 ST、退市整理、流动性极差和无明确催化标的。
...
| V6 | V7 |
|-----|-----|
| 三角色辩论（猎手/账房/守夜人） | 四角色辩论（+产业链研究员） |
| 纯技术面+基本面分析 | 增加 Serenity 式产业链瓶颈分析 |
| 本地开发 | GitHub 开源发布 |

### 🆕 新功能

- **产业链研究员角色**: 第 4 个 AI 辩论角色，实现 Serenity 式供应链瓶颈分析方法论
- **8 层产业链价值链地图**: 终端客户→系统集成→模组→芯片→工艺→设备→材料→基础设施
- **16 个 A 股主题卡点**: AI半导体/CPO光通信/机器人/AI基建等常见主题的典型瓶颈环节
- **7 维评分卡**: 需求确定性/传导清晰度/业务纯度/市值弹性/市场忽视度/验证速度/下行风险
- **10 个红旗风险信号**: 单一客户依赖/社交媒体炒作/融资风险/存货应收异常/毛利率不改善等
- **证据等级分级**: 强证据/中等证据/弱证据三级体系
- **盘前卡点预检**: 开盘前扫描产业链卡点，注入辩论引擎
- **飞书报告卡点章节**: 策略报告新增「🔬 产业链卡点分析」区块

### 🔧 改进

- 辩论引擎从 3 角色升级为 4 角色并行调用
- 裁决器现在综合四角色观点
- 启动时自动补跑错过的盘前任务
- 测试覆盖从 57 → 76 个（+20 个 serenity 专项测试）

### 🐛 Bug 修复

- 修复 _catchup_missed_jobs 函数名引用错误
- 修复 workshop.py 决策字典缺少卡点字段的问题

### 📦 发布

- GitHub 仓库: https://github.com/jarrensea-lab/bcku
- 分支: main
- 许可证: MIT
