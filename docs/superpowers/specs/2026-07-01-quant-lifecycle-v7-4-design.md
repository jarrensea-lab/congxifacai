# 恭喜发财 v7.4.0-dev 设计：量化生命周期与分钟级触发底座
...
- 盘中必须支持分钟级触发；秒级能力作为 v7.5/v8 的架构预留，不在 v7.4 假装完成。
...
3. v7.4.0-dev.3：盘中分钟级扫描与飞书提醒。
4. v7.4.0-dev.4：盘前/盘中/收盘报告改为池生命周期报告。
